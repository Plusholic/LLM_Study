[원문링크](https://arxiv.org/abs/2112.10752)

# 3 LINE SUMMARY

- 기존의 이미지 생성을 위한 Diffusion Model은 주로 Pixel Space에서 동작하여 많은 계산과정이 필요하였음. 이를 해결하기 위해 Latent Space에서 Diffusion Model을 적용하는 방법을 제안함
- 이미지를 오토인코더를 통해서 잠재 공간으로 다운샘플링 하고, 여기에 점진적으로 노이즈를 추가한 후 U Net을 통해 제거하는 과정을 저친 후 다시 원본 이미지로 디코딩하여 재구성하는 방식.
- 인패인팅, 이미지 생성, 이미지 합성, 업스케일링 등에서 GAN 등의 기존 모델과 비교했을 때 최첨단 성능을 달성하면서 계산량은 줄어듬

# Abstract

> 이미지 생성을 위한 최첨단 방법 중 하나인 확산 모델(Diffusion Models, DMs)을 소개. `기존의 DMs는 주로 픽셀 공간에서 작동`하여 높은 GPU 자원을 소비하고, 추론 과정이 복잡하여 비싸다는 단점. 이러한 문제를 해결하기 위해, 이 논문은 강력한 사전 학습된 `오토인코더의 잠재 공간(latent space)에서 확산 모델을 적용하는 방법을 제안`. 이를 통해 복잡성을 줄이고 세부 사항을 보존하여 시각적 충실도를 크게 향상시킬 수 있음. 또한, Cross-Attention Layers를 모델 구조에 도입하여 텍스트나 바운딩 박스와 같은 일반적인 조건 입력을 위한 강력하고 유연한 생성기를 구현. 제안된 잠재 확산 모델(Latent Diffusion Models, LDMs)은 이미지 인페인팅(image inpainting), 클래스 조건부 이미지 생성(class-conditional image synthesis) 및 다양한 작업에서 새로운 최첨단 성능을 달성하면서도, 기존의 픽셀 기반 DMs에 비해 계산 요구 사항을 줄임.
> 
...
- 이를 통해 하고, 텍스트나 경계 상자와 같은 조건 입력을 처리할 수 있는 강력한 제너레이터를 구현.
...
- `고정된 길이 $T$의 Markov Chain의 역과정을 학습`하는 것에 해당
...
- $\epsilon_\theta(x_t, t)$: 노이즈가 추가된 입력 $x_t$와 $t$가 주어졌을 때 노이즈를 제거한 예측값
...
$$
L_{\text{LDM}} := \mathbb{E}_{{\mathcal{E}(x), \epsilon \sim \mathcal{N}(0, 1), t}} [| \epsilon - \epsilon_\theta (z_t, t) |^2_2]
$$
...
- 다양한 다운샘플링 팩터 $f \in {1, 2, 4, 8, 16, 32}$ 사용 시, LDM(Latent Diffusion Models)의 성능 분석.
    - 여기서 $\text{LDM-}f$는 다운샘플링 팩터 $f$를 의미
    - $f = \frac{H}{h} = \frac{W}{w}=1$이면, $H=h, W=w$이기 때문에 픽셀 기반의 Diffusion Models임
...
- **컨볼루션 샘플링 설명**
    - LDMs은 입력 정보에 공간적으로 정렬된 조건 정보를 연결하여 효율적인 범용 이미지-이미지 변환 모델로 사용할 수 있음.
    ...
    - Symentic Synthesis의 경우, 풍경 이미지와 해당 시멘틱 맵을 사용하여 트레이닝하며, 입력 해상도는 256²입니다(384²에서 크롭된 부분을 사용).
...
> ImageNet 데이터셋을 사용한 초해상도 생성 실험에서는 LDM이 다양한 저해상도 이미지를 고해상도로 변환하는 성능을 평가합니다. LDM-4는 기존 방법들에 비해 우수한 FID와 PSNR 점수를 기록.
> 
- SR3의 데이터 처리 파이프라인을 따라 이미지를 4배 다운샘플링하는 bicubic interpolation을 사용.
- Open Images 데이터셋에서 사전 훈련된 $f = 4$ 오토인코더 모델을 사용.
- 저해상도 조건 $y$와 UNet의 입력을 연결하여 사용했습니다. 이때 $\tau_\theta$는 신원(identity)으로 간주.
- 정성적 및 정량적 결과에서 LDM-SR이 FID 점수에서 SR3를 능가했습니다.
- 하지만 SR3는 IS 점수에서 더 나은 결과를 보임.
...
- **제한 사항:**
    - LDMs(Latent Diffusion Models)은 픽셀 기반 접근 방식에 비해 계산 요구 사항을 크게 줄이지만, 샘플링 과정이 여전히 GANs(Generative Adversarial Networks)보다 느림.
    - 또한 높은 정밀도가 요구되는 경우 LDMs의 사용이 문제가 될 수 있음.
    - 비록 $f = 4$ 오토인코딩 모델에서 이미지 품질 손실이 매우 적다고 하더라도, 픽셀 단위의 정확도가 필요한 작업에서는 재구성 능력이 병목이 될 수 있음.
    - 초고해상도 모델은 이미 이와 같은 한계가 일부 존재하는 것으로 가정.
- **사회적 영향:**
    - 생성 모델은 이중적 측면을 가집니다.
        - 한편으로는 다양한 창의적 응용을 가능하게 하고 기술에 접근하는 비용을 줄임으로 민주화될 수 있음.
        - 다른 한편으로는 조작된 데이터를 만들어 유포하거나 허위 정보를 퍼뜨리는 등의 부작용을 초래할 수 있음.
    - 특히 이미지의 고의적인 조작("딥페이크")은 여성에게 특히 많이 발생하는 문제.
    - 생성 모델은 훈련 데이터도 드러낼 수 있습니다. 이는 데이터에 민감하거나 개인적인 정보가 포함되어 있을 때 큰 문제가 될 수 있음.
    - 또한, 딥러닝 모델들은 기존 데이터에 존재하는 편향을 재현하거나 악화시키는 경향이 있음.
    - 확산 모델은 GAN 기반 접근법보다 데이터 분포를 더 잘 포괄하지만, 우리의 이중 단계 접근법(GAN과 가능도 기반 목표를 결합한 것)이 데이터를 잘못 표현할 가능성이 있는지 연구가 필요함.

## Conclusion

- 이 논문에서는 노이즈 제거 확산 모델(denoising diffusion models)의 훈련 및 샘플링 효율성을 크게 향상시키는 잠재 확산 모델(Latent Diffusion Models, LDMs)을 제안.
- 제안된 LDMs는 모델의 품질을 저하시키지 않으면서도 효율성을 높이는 간단하고 효과적인 방법을 제공함. 또한 크로스 어텐션 조건 메커니즘(cross-attention conditioning mechanism)을 기반으로, 다양한 조건부 이미지 합성 작업에서 최첨단 성능을 달성.
- 제안된 접근 방식은 특정 작업에 특화된 아키텍처 없이도 뛰어난 성능을 보여주었음.